...
def content2attribute_sentences(sentences, stem_list):
    ...
    for sentence in sentences:
        for word in word_tokenize(sentence):
            if stem_list.get(porter_stemmer.stem(word)) is not None:
                # text_no_small_words = re.sub(r'\b\w{1,2}\b', '', sentence)

                remove = str.maketrans(string.punctuation, "{:<32}".format(""))
                text_no_small_words = sentence.translate(remove)

                text_no_small_words = re.sub(r"[^a-zA-Z0-9\s\'\"]", ' ', text_no_small_words)
